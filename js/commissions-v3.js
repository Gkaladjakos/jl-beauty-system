// Commissions Module - Version 3 corrigée
const CommissionsV3 = {
    data: [],
    coiffeuses: [],
    ventes: [],
    ...
    updateStats() {
        ...
        const commissionsPaid    = filteredData.filter(c => c.statut === 'Payé').reduce((sum, c) => sum + (c.total_commission || 0), 0);
        const commissionsAttente = filteredData.filter(c => c.statut === 'Calculé' || c.statut === 'En attente').reduce((sum, c) => sum + (c.total_commission || 0), 0);
        ...
        tbody.innerHTML = dataToRender.map(comm => {
            ...
            return `
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4">
                        <p class="font-medium text-gray-800">${comm.coiffeuse_nom || 'N/A'}</p>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <p class="font-bold text-purple-600">${Utils.formatCurrency(comm.total_commission || 0)}</p>
                    </td>
                    <td class="px-6 py-4">
                        ${isPaid
                        ...
                        <div class="flex space-x-2">
                            ${!isPaid ? `
                                <button onclick="CommissionsV3.markAsPaid('${comm.id}')"
                                        class="text-green-600 hover:text-green-800" title="Marquer comme payé">
    ...
    async markAsPaid(commissionId) {
        ...
        if (!confirm(`Confirmer le paiement de ${Utils.formatCurrency(commission.total_commission)} à ${commission.coiffeuse_nom} ?`)) return;

        try {
            App.showLoading();
            await Utils.update('commissions', commissionId, {
                statut:         'Payé',
                // ✅ FIX 1 : ISO string au lieu de Date.now()
    ...
    async markAllAsPaid() {
        ...
        if (!confirm(`Marquer ${unpaidCommissions.length} commission(s) comme payées pour un total de ${Utils.formatCurrency(totalAmount)} ?`)) return;

        try {
            App.showLoading();
            // ✅ FIX 2 : ISO string au lieu de Date.now()
    ...
    showDetails(commissionId) {
        ...
        // ✅ FIX 7 : parse conditionnel — Supabase JSONB retourne déjà un objet
        let details = [];
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div>
                        ...
                        <p class="font-bold">${Utils.formatCurrency(commission.total_ventes || 0)}</p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-600">Commission totale</p>
                        <p class="font-bold text-purple-600 text-xl">${Utils.formatCurrency(commission.total_commission || 0)}</p>
                    </div>
                    <div>
                                ...
                                <tbody>
                                    ${details.map(d => `
                                        <tr class="border-b">
                                            ...
                                            <td class="px-3 py-2 text-right">${Utils.formatCurrency(d.montant || 0)}</td>
                                            <td class="px-3 py-2 text-right font-bold">${Utils.formatCurrency(d.commission || 0)}</td>
                                        </tr>`).join('')}
                                </tbody>
                            </table>
                        </div>
    ...
    printReceipt(commissionId) {
        ...
        // ✅ FIX 8 : même parse conditionnel que showDetails
        let details = [];
        if (Array.isArray(commission.details)) {
            details = commission.details;
        } else if (typeof commission.details === 'string') {
        ...
        const receiptHTML = `
            ...
            <table>
                <thead>
                    <tr><th>Service</th><th>Montant</th><th>Commission</th></tr>
                </thead>
                <tbody>
                    ${details.map(d => `
                        <tr>
            ...
            <div class="total">
                <p>Total des ventes : ${Utils.formatCurrency(commission.total_ventes || 0)}</p>
                <p>Commission totale : ${Utils.formatCurrency(commission.total_commission || 0)}</p>
            </div>
