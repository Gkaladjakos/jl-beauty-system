...
const CommissionsV3 = {
                ...
                <div class="flex justify-between items-center">
                    <div class="flex space-x-3">
                        ...
                        <button id="btn-pay-all"
                                ...
                                disabled
                                title="Sélectionnez d'abord une période">
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium
                                           text-gray-500 uppercase">Période</th>
                                <th class="px-6 py-3 text-left text-xs font-medium
                                           text-gray-500 uppercase">Services Rendus</th>
                                <th class="px-6 py-3 text-left text-xs font-medium
                                           text-gray-500 uppercase">Commission</th>
                                <th class="px-6 py-3 text-left text-xs font-medium
                                           text-gray-500 uppercase">Statut</th>
                                <th class="px-6 py-3 text-left text-xs font-medium
                                           text-gray-500 uppercase">Date Paiement</th>
                                <th class="px-6 py-3 text-left text-xs font-medium
    ...
    // ================================================================
    // Bouton "Payer la période" — état dynamique
    // ================================================================
    ...
    updateStats() {
        ...
        document.getElementById('commission-payees').textContent  =
            Utils.formatCurrency(commissionsPaid);
        document.getElementById('commission-attente').textContent =
            Utils.formatCurrency(commissionsAttente);
        document.getElementById('coiffeuses-count').textContent   =
            uniqueCoiffeuses;
    },
    ...
    // ================================================================
    // markAsPaid — INDIVIDUEL avec modal de confirmation
    // ================================================================
            ...
            <div class="space-y-4">
                ...
                <div class="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <div class="flex items-center space-x-3 mb-3">
                        ...
                        <div>
                            <p class="font-bold text-green-800">Confirmer le paiement</p>
                    ...
                    <div class="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div class="bg-white rounded p-2">
                            ...
                            <p class="font-semibold">
                                ${commission.nombre_services || 0} service(s)
                            </p>
                        </div>
                        <div class="bg-white rounded p-2">
            ...
            async (modal) => {
                ...
                if (!paymentDate) {
                    App.showNotification(
                        'Veuillez sélectionner une date de paiement', 'error'
                    );
                    return;
                }
                ...
                if (new Date(paymentDate) > new Date()) {
                    App.showNotification(
                        'La date de paiement ne peut pas être dans le futur', 'error'
                    );
                    return;
                }
    ...
    async markAllAsPaid() {
        ...
        if (!this.currentPeriod) {
            App.showNotification(
                'Veuillez sélectionner une période avant de payer', 'error'
            );
            return;
        }
        ...
        if (unpaid.length === 0) {
            App.showNotification(
                'Toutes les commissions de cette période sont déjà payées', 'info'
            );
            return;
        }
        ...
        const modalContent = `
            <div class="space-y-4">
                ...
                <div class="flex items-start space-x-2 p-3
                    ...
                    <i class="fas fa-exclamation-triangle text-yellow-500 mt-0.5
                              flex-shrink-0"></i>
        ...
        Utils.createModal(
            ...
            async (modal) => {
                ...
                try {
                    ...
                    if (errors.length === 0) {
                        App.showNotification(
                            `✅ ${successCount} commission(s) payée(s) — ${periodLabel}`,
                            'success'
                        );
                    } else {
                        App.showNotification(
                            `⚠️ ${successCount} réussie(s), ${errors.length} erreur(s) : ` +
                            errors.join(', '),
                            'error'
                        );
                    }
...
// ✅ Recalcule toujours depuis les ventes (corrige les anciennes aussi)
// ✅ Formule : montant × 0.93 × taux%
// ================================================================
...
async calculerCommissions() {
    ...
    try {
        ...
        if (salesInPeriod.length === 0) {
            App.hideLoading();
            App.showNotification(
                'Aucune vente de service dans cette période', 'info'
            );
            return;
        }
        ...
        salesInPeriod.forEach(vente => {
            ...
            if (items.length > 0) {
                items.forEach(item => {
                    ...
                    const taux       = item.taux_commission || 0;

                    // ✅ NOUVELLE FORMULE — recalcul systématique
                    const commission = Math.round(montant * 0.93 * taux / 100 * 100) / 100;
